...
SQLHighlighter::SQLHighlighter (class QTextDocument *parent)
  ...
  {   // quoted strings
    ...
    rules.push_back (Rule (QStringLiteral ("\".*\""), quotationFormat) );
    rules.push_back (Rule (QStringLiteral ("'.*'"), quotationFormat) );
  }
  ...
  {   // primary keywords
    ...
    QString str = QStringLiteral ("begin commit rollback select from where explain insert into values update create delete drop grant revoke lock set truncate as join on table view order group by having");

    str.replace (QStringLiteral (" "), QStringLiteral ("|"));

    rules.push_back (Rule (QStringLiteral ("\\b(") + str + QStringLiteral (")\\b"), keywordFormat) );
  }
